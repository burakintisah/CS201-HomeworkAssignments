...
void displayAllStudents(SRS S) {
	...
	for (int i = 0; i < studentNo; i++) {
		...
		cout << allStudents[i].getStudentId() << ")";
		Course * temp = allStudents[i].getCourses();
		for (int k = 0; k < allStudents[i].getCourseNo(); k++) {
			if (temp[k].getCourseId() == 201) {
				string tempGrade = temp[k].getCourseGrade();
				cout << "grade " << tempGrade << endl;
			}
		}
	}
	if (allStudents != NULL)
